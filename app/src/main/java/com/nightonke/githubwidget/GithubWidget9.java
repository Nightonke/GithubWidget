 ...
 */
public class GithubWidget9 extends AppWidgetProvider {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (AppWidgetManager.getInstance(context).getAppWidgetIds(
                new ComponentName(context, GithubWidget9.class)).length == 0) return;

        if (BuildConfig.DEBUG) Log.d("GithubWidget", "Receive in widget 9: " + intent.getAction());

        switch (intent.getAction()) {
            case Actions.UPDATE_FROM_SERVICE:
                if (BuildConfig.DEBUG)
                    Log.d("GithubWidget", "Receive in widget 9: Update from service");
                updateAll(context, -1);
                break;
            case Actions.CLICK_AVATAR:
                if (BuildConfig.DEBUG)
                    Log.d("GithubWidget", "Receive in widget 9: Update from manual");
            ...
            case Actions.UPDATE_MOTTO:
                if (BuildConfig.DEBUG)
                    Log.d("GithubWidget", "Receive in widget 9: Update motto");
                Util.updateMotto(GithubWidget9.class, R.layout.github_widget_9, context,
                        (int) (Util.getScreenWidth(context)
                                - Util.getDimen(R.dimen.github_widget_9_avator_size)),
                        (int )Util.getDimen(R.dimen.github_widget_9_avator_size));
                break;
    ...
    private void updateAll(Context context, int appWidgetId) {
        ...
        RemoteViews remoteViews
                = new RemoteViews(context.getPackageName(), R.layout.github_widget_9);
        ComponentName componentName = new ComponentName(context, GithubWidget9.class);
        ...
        new ListViewContentTask(remoteViews, context, componentName,
                appWidgetId, Widget.WIDGET_9, this.getClass())
                .executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, "");
